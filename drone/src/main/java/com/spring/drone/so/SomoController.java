...
public class SomoController {
	...
	public ModelAndView somoim_detail(SomoimVO somoimVO, SomoimPostVO somoimPostVO, HttpServletResponse response){
		ModelAndView result = new ModelAndView();
		...
		/*somoimVO = somoimDAOService.verifyFounder(somoimVO);
		if (somoimVO.getFounder().equals(somoimPostVO.getId())) {
			ArrayList<SomoimPostVO> somoimPostList = somoimDAOService.getSomoimPostList(num);
			result.addObject("somoimPostList", somoimPostList);
			result.addObject("somoimVO", somoimVO);
			result.setViewName("so/detail");
			...
			writer.write("<script> alert('회원만 열람하실 수 있는 소모임입니다.');location.href='template.templ?page=main.co';</script>");
